...
| 범위 밖 | 이유 |
|--------|------|
| 외부 침투(North-South) 탐지 | 기존 IDS/WAF 영역, 연구 범위 중복 |
| 취약점 익스플로잇 탐지 | 별도 연구 영역 |
| 클라우드 제공자 레이어(AWS, GCP) | 커널 접근이 제한된 환경은 별도 연구 필요 |
| 실시간 자동 대응(Response) | 탐지에 집중, 대응은 후속 연구 |
